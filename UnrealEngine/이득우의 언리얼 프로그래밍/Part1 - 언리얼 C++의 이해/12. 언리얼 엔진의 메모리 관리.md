...
사실 내가 이해가 안됐던 부분은 다음과 같다.

`UPROPERTY`로 선언한 `PropStudent`와 그냥 선언된 `NonPropStudent`가 있을 때
Shutdown시 로그에서 `PropStudent`가 "유효한 오브젝트", `NonPropStudent`가 "유효하지 않은 오브젝트"라고 출력됐다.
...
이에 대한 답은 이렇다. 언리얼 오브젝트가 유효(Valid)하다는 것은 메모리 누수와 무관하게, 오브젝트가 언리얼 엔진에 의해서 
...
만약 잘 관리되고 있다면(Valid하다면) 언리얼 엔진의 가비지 컬렉터가 적절한시점에 메모리를 회수해줄 것이기 때문에
사용자는 객체의 메모리 및 누수 현상에 신경을 쓸 필요가 없게된다.
...
회수해 갔는지 안갔는지에 의의를 두기보다는 ***널 값만으로 오브젝트의 유효성을 판단해서는 안된다는 것이다.***


그니까 유효하지 않은 언리얼 오브젝트라는 건, 가비지 컬렉터에 의해 메모리가 회수됐다는 의미가 아니라
