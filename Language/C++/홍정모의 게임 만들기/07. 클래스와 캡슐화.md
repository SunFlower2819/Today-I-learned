...
### 🔵문제 발생
아래의 코드에는 문제가 있다. 바로 모든 집이 다른 색으로 표현되는 것이 아니라, 모두가 같은 랜덤 색으로 표현이 된다는 것이다.
...
아니면 'update()함수에서 집을 색을 세팅해주고 집을 그리기 때문이지 않을까?' 라는 생각도 든다.
...
**아래의 코드를 더 객체지향적으로 바꾼 코드가 있으니까 꼭 더 아래로 내려보기**
